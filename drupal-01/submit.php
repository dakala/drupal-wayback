...
function submit_enter() {
  ...
  $output .= "<P>\n <B>Your name:</B><BR>\n";
  $output .= format_username($user && $user->userid);
  $output .= "</P>\n";
  ...
  $output .= " <TEXTAREA WRAP=\"virtual\" COLS=\"50\" ROWS=\"10\" NAME=\"content\" MAXLENGTH=\"20\"></TEXTAREA><BR>\n";
...
function submit_preview($subject, $content, $article, $category) {
  ...
  $output .= "<P>\n";
  $output .= "<B>Content:</B><BR>\n";
  $output .= " <TEXTAREA WRAP=\"virtual\" COLS=\"50\" ROWS=\"10\" NAME=\"content\">". check_textarea($content) ."</TEXTAREA><BR>\n";
  $output .= " <SMALL><I>Allowed HTML tags: ". htmlspecialchars($allowed_html) .".</I></SMALL>\n";
  $output .= "</P>\n";
  ...
  if (empty($subject)) {
    ...
    $output .= "<P>\n";
    $output .= " <FONT COLOR=\"red\"><B>Warning:</B></FONT> you did not supply an <U>content</U>.\n";
    $outout .= "</P>\n";
    $output .= "<P>\n";
    $output .= " <INPUT TYPE=\"submit\" NAME=\"op\" VALUE=\"Preview submission\">\n";
    $output .= "</P>\n";
  ...
  $theme->header();
  $theme->article(new Story($user->userid, $subject, $content, $article, $category, time()));
  $theme->box("Submit a story", $output);
  $theme->footer();
}

function submit_submit($subject, $content, $article, $category) {
  global $user, $theme;
  ...
  db_query("INSERT INTO stories (author, subject, content, article, category, timestamp) VALUES ('$user->id', '". check_input($subject) ."', '". check_input($content) ."', '". check_input($article) ."', '". check_input($category) ."', '". time() ."')");
...
// dakala: ????
global $subject, $content, $article, $category, $op;

include "includes/common.inc";

switch($op) {
  case "Preview submission":
    submit_preview($subject, $content, $article, $category);
    break;
  case "Submit submission":
    submit_submit($subject, $content, $article, $category);
    break;
  default:
    submit_enter();
    break;
