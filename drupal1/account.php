...
function account_validate($user) {
  ...
  if (empty($user['real_email']) || (!preg_match('/^[_\.0-9a-z-]+@([0-9a-z][0-9a-z-]+\.)+[a-z]{2,3}$/i', $user['real_email']))) $error .= "<LI>the specified e-mail address is not valid.</LI>\n";
  if (empty($user['userid']) || (preg_match('/[^a-zA-Z0-9_-]/', $user['userid']))) $error .= "<LI>the specified username is not valid.</LI>\n";
